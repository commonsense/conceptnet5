...
ARG CONCEPTNET_DB_HOST=127.0.0.1
...
ENV CONCEPTNET_DB_HOST=$CONCEPTNET_DB_HOST

RUN apt-get update
RUN apt-get install -y build-essential python3-pip python3-dev libhdf5-dev libmecab-dev mecab-ipadic-utf8 postgresql postgresql-contrib

RUN mkdir data

RUN PGPASSWORD=$CONCEPTNET_DB_PASSWORD psql -h $CONCEPTNET_DB_HOST -U $CONCEPTNET_DB_USER
