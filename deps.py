...
def extract_wiktionary(deps):
    for lang in wiktionary_langs:
        ...
        outputs = [prefix+'extracted/wiktionary/%s/wiktionary_%02d.msgpack'%(lang, i)
...
def parse_wiktionary(deps):
    for lang in wiktionary_langs:
        for input in deps['extract %s wiktionary'%lang]['outputs']:
            output = input.replace('extracted', 'edges')
            index = input[-10:-8]

            deps['parse %s wiktionary %s'%(lang, index)] = {
                'inputs': [input],
                'outputs': [input.replace('extracted', 'edges')],
...
def msgpack_to_csv(deps):
    ...
    for k,v in deps.items():
        if not k.startswith('parse'): continue
...
def combine_assertions(deps):
    new_deps = {}
    for k,v in deps.items():
        if not k.startswith('count and rank'): continue
...
def build_db(deps):
    inputs = []
    for k,v in deps.items():
        if not k.startswith('combine assertions'): continue
        inputs += v['outputs']
